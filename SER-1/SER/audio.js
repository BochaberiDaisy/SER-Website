const MODEL_PATH = "./model1.tflite"
...
async function detect(audioBlob){
    if (!SER) {
        SER = await tflite.loadTFLiteModel(MODEL_PATH);
    }
}
...
async function query(file) {
	...
	const response = await fetch(
		...
		{
			headers: { Authorization: "Bearer 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" },
			method: "POST",
			body: data,
		}
	);
      ...
      return (
        navigator.mediaDevices
          ...
          .then((stream) /*of type MediaStream*/ => {
            ...
            //add a data available event listener in order to store the audio data Blobs when recording
  ...
  stop: function () {
    ...
    return new Promise((resolve) => {
      ...
      audioRecorder.mediaRecorder.addEventListener("stop", () => {
        ...
        resolve(audioBlob);
        query(audioBlob).then((response) => {
            console.log(JSON.stringify(response));
            results = JSON.stringify(response);
            const genesisResults = JSON.parse(results)
            const results_ = Object.values(genesisResults)[0]
            console.log(typeof(results_))
            console.log(results_)
            score = results_["score"]
            emotion = results_["label"]
            ...
            score_num = (score*100).toFixed(2);
            console.log(score_num)
            console.log(score)
            console.log(emotion)

            span1.textContent = emotion
            span2.textContent = score_num
        });
      });
      audioRecorder.cancel();
    });
